"use client"
import { BuscarResumosPendentes } from "@/services/map/Resumos";
import { BuscarUsuarios } from "@/services/usuarios";
import Link from "next/link";
import { useEffect, useState } from "react";

export default function ListResumosPendentes() {
    ...
    useEffect(() => {
        async function fetchIdeiasPendentes() {
            try {
                const ideiasPendentes = await BuscarResumosPendentes();
    ...
    return (
        <div className="flex flex-col w-10/12 h-max gap-5 bg-white rounded-md ">
            <h2 className="font-semibold text-lg text-start p-5">
                Resumos pendentes de pontuação
            </h2>
